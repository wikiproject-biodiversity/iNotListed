...
# --------------------------
# Provenance footer (shared across markdown / PDF / issue comments)
# --------------------------
_CODEBERG_BASE = "https://codeberg.org/wikiproject-biodiversity/iNotListed"
_GITHUB_BASE = "https://github.com/wikiproject-biodiversity/iNotListed"
_TRANSLATED_READMES = [
    ...
    ("മലയാളം", "README.ml.md"),
    ("Igbo", "README.ig.md"),
    ("Dagbanli", "README.dag.md"),
]


def _provenance_section() -> str:
    lang_links = " · ".join(
        f"[{label}]({_CODEBERG_BASE}/src/branch/main/{path})"
        for label, path in _TRANSLATED_READMES
    )
    return (
        "\n---\n\n"
        "## About this report\n\n"
        f"Generated by [iNotListed]({_CODEBERG_BASE}) "
        f"(also mirrored on [GitHub]({_GITHUB_BASE})). "
        "Part of "
        "[WikiProject Biodiversity](https://www.wikidata.org/wiki/Wikidata:WikiProject_Biodiversity).\n\n"
        "**Wikiblitz background**\n"
        "- https://doi.org/10.37044/osf.io/5ue2s_v1\n"
        "- https://doi.org/10.37044/osf.io/7s6da_v1\n\n"
        f"**README in other languages**: {lang_links}\n"
    )
...
def generate_markdown_report(search_value, search_type="project", languages=None, output_folder="reports"):
    ...
    md_lines.append(_provenance_section())
